...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgres://postgres:123@localhost:5432/fyyurapp'
...
# Return all the venues depending on city and state.
  outcome = db.session.query(Venue).with_entities(Venue.city,Venue.state).group_by(Venue.city,Venue.state).all()
...
# Will return venues names and it is case-insensitive using .ilike(f'%{search_term}%').
  outcome = db.session.query(Venue).filter(Venue.name.ilike(f'%{search_term}%')).all()
...
# Will return the Shows depending on venue_id using join Show and Artist tables.
  outcome = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).all()
...
# Will return artists names and it is case-insensitive using .ilike(f'%{search_term}%').
  artist = db.session.query(Artist).filter(Artist.name.ilike(f'%{search_term}%')).all()
...
# Will return the Shows depending on artist_id using join Show and Venue tables.
  show = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).all()
# Will return the time for right now.  
  present_time = datetime.now()
